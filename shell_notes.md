...
## stripe
```
The tilde reflects the home path. You can list files and folders in the home directory even if your current directory is elsewhere
...
myfile.txt
```
That is, because you're already in the directory you don't need to specify the file's path. The *absolute path* for the file, however, is 
...
The ```..``` notation shifts you up one level to the parent directory. So, if you wanted to list the contents of the parent directory you can use
...
```
You can also navigate using this notation, or using relative and absolute paths. To change directories you use the ```cd``` command. 
